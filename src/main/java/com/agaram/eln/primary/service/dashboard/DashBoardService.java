...
//			lstlimscompleted.addAndGet(countcompleted);
				...
				if (!lstordersdem.isEmpty()) {
					lstorders.addAll(lstordersdem.stream()
							.map(lsOrderDetail -> new Logilabordermaster(lsOrderDetail.getBatchcode(),
									...
									lsOrderDetail.getOrdersaved(),lsOrderDetail.getRepeat(),lsOrderDetail.getLsautoregisterorders(),lsOrderDetail.getSentforapprovel(),lsOrderDetail.getApprovelaccept(),lsOrderDetail.getAutoregistercount()))
							.collect(Collectors.toList()));
				}
			} else {
				List<LSlogilablimsorderdetail> lstordersdem = lslogilablimsorderdetailRepository
				...
				lstorders = lstordersdem.stream()
						.map(lsOrderDetail -> new Logilabordermaster(lsOrderDetail.getBatchcode(),
								...
								lsOrderDetail.getLsautoregisterorders(),lsOrderDetail.getSentforapprovel(),lsOrderDetail.getApprovelaccept(),lsOrderDetail.getAutoregistercount()))
						.collect(Collectors.toList());
			}
...
//									"R", 3, objuser, fromdate, todate, 3, lstproject, pageable));
					...
					lstordersdem = lslogilablimsorderdetailRepository.getLSlogilablimsorderdetaildashboardforcompleted(
...
//										lstsample, fromdate, todate, searchkeywords, objuser);
						...
						lstorders = lsResults.stream()
								.map(lsOrderDetail -> new Logilabordermaster(lsOrderDetail.getBatchcode(),
										...
										lsOrderDetail.getSentforapprovel(),lsOrderDetail.getApprovelaccept(),lsOrderDetail.getAutoregistercount()))
								.collect(Collectors.toList());
